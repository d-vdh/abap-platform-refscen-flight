...
CLASS ltcl_handler DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    CONSTANTS:
      ...
      fm_set_booking TYPE sxco_fm_name VALUE 'ZAI_DMOFLIGHT_TRAVEL_SET_BOOKING',
    ...
    METHODS:
      "! Checks if { @link ..lhc_travel.METH:create } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_CREATE } succeeds.
      create_success            FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:create } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_CREATE } fails.
      create_fail               FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:update } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_UPDATE } succeeds.
      update_success            FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:update } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_UPDATE } fails.
      update_fail               FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:delete } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_DELETE } succeeds.
      delete_success            FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:delete } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_DELETE } fails.
      delete_fail               FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:read } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_READ } succeeds.
      read_success              FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:read } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_READ } fails.
      read_fail                 FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:rba_Booking } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_READ } succeeds.
      rba_success               FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:rba_Booking } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_READ } fails.
      rba_fail                  FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:cba_Booking } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_READ } and { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_UPDATE } both succeeds.
      cba_success               FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:cba_Booking } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_READ } fails.
      cba_fail_read             FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:cba_Booking } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_READ } succeeds but { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_UPDATE } fails.
      cba_fail_update           FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lhc_travel.METH:set_status_booked } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_SET_BOOKING } succeeds.
      set_status_booked_success FOR TESTING RAISING cx_static_check,
      "! Checks if { @link ..lhc_travel.METH:set_status_booked } works as expected when { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_SET_BOOKING } fails.
...
CLASS ltcl_handler IMPLEMENTATION.
  ...
  METHOD create_success.
    DATA:
      entities    TYPE TABLE FOR CREATE ZAI_DMOi_travel_u\\travel.


    DATA(fm_create_double) = fm_test_environment->get_double( fm_create ).

    DATA(travel) = VALUE ZAI_DMOtravel( travel_id = travel_id ).
  ...
  METHOD update_success.
    DATA:
      entities    TYPE TABLE FOR UPDATE ZAI_DMOi_travel_u\\travel.

    DATA(fm_update_double) = fm_test_environment->get_double( fm_update ).

    DATA(travel)  = VALUE ZAI_DMOs_travel_in(  travel_id = travel_id  agency_id = agency_id  customer_id = customer_id ).
    DATA(travelx) = VALUE ZAI_DMOs_travel_inx( travel_id = travel_id  agency_id = abap_true  customer_id = abap_true   ).
  ...
  METHOD delete_success.
    DATA:
      keys    TYPE TABLE FOR DELETE ZAI_DMOi_travel_u\\travel.
  ...
  METHOD read_success.
    DATA:
      keys        TYPE TABLE FOR READ IMPORT ZAI_DMOi_travel_u\\travel,
      result      TYPE TABLE FOR READ RESULT ZAI_DMOi_travel_u\\travel,
      result_line TYPE STRUCTURE FOR READ RESULT ZAI_DMOi_travel_u\\travel.
  ...
  METHOD rba_success.
    DATA:
      keys_rba               TYPE TABLE     FOR READ IMPORT ZAI_DMOi_travel_u\\travel\_Booking,
      result                 TYPE TABLE     FOR READ RESULT ZAI_DMOi_travel_u\\travel\_Booking,
      result_line            TYPE STRUCTURE FOR READ RESULT ZAI_DMOi_travel_u\\travel\_Booking,
      association_links      TYPE TABLE     FOR READ LINK   ZAI_DMOi_travel_u\\travel\_Booking,
      association_links_line TYPE STRUCTURE FOR READ LINK   ZAI_DMOi_travel_u\\travel\_Booking,
  ...
  METHOD cba_success.
    DATA:
      entities_cba        TYPE TABLE FOR CREATE ZAI_DMOi_travel_u\\travel\_booking,
      mapped_line_booking LIKE LINE OF mapped-booking,
      travel              TYPE ZAI_DMOs_travel_in,
      travelx             TYPE ZAI_DMOs_travel_inx,
      old_booking         TYPE ZAI_DMObooking,
      new_booking         TYPE ZAI_DMOs_booking_in,
      new_bookingx        TYPE ZAI_DMOs_booking_inx,
      old_bookings        TYPE ZAI_DMOt_booking,
      new_bookings        TYPE ZAI_DMOt_booking_in,
      new_bookingsx       TYPE ZAI_DMOt_booking_inx.

    travel  = VALUE ZAI_DMOs_travel_in(  travel_id = travel_id ).
    travelx = VALUE ZAI_DMOs_travel_inx( travel_id = travel_id ).

    new_booking  = VALUE ZAI_DMOs_booking_in(
                       travel_id   = travel_id
    ...
    old_booking  = VALUE ZAI_DMObooking(
                       travel_id   = travel_id
                       booking_id  = booking_id
                       customer_id = customer_id
                       carrier_id  = carrier_id
    ...
    new_bookingx = VALUE ZAI_DMOs_booking_inx(
                       booking_id  = booking_id
  ...
  METHOD cba_fail_read.
    ...
    DATA(new_booking) = VALUE ZAI_DMOs_booking_in(
                            travel_id   = travel_id
                            booking_id  = booking_id  + 1
                            customer_id = customer_id + 1
                            carrier_id  = carrier_id
  ...
  METHOD get_instance_feature.
    TYPES:
      BEGIN OF check_line,
        travel_id                     TYPE ZAI_DMOtravel-travel_id,
        status                        TYPE ZAI_DMOtravel-status,
    ...
    DATA:
      keys               TYPE TABLE     FOR INSTANCE FEATURES KEY     ZAI_DMOi_travel_u\\travel,
      requested_features TYPE STRUCTURE FOR INSTANCE FEATURES REQUEST ZAI_DMOi_travel_u\\travel,
      result             TYPE TABLE     FOR INSTANCE FEATURES RESULT  ZAI_DMOi_travel_u\\travel,
      result_line        TYPE STRUCTURE FOR INSTANCE FEATURES RESULT  ZAI_DMOi_travel_u\\travel,
      import_read        TYPE STRUCTURE FOR READ              IMPORT  ZAI_DMOi_travel_u\\travel,
      import_read_tab    TYPE TABLE     FOR READ              IMPORT  ZAI_DMOi_travel_u\\travel,
      result_read        TYPE TABLE     FOR READ              RESULT  ZAI_DMOi_travel_u\\travel.
...
*                           ).
*    DATA(env_config) = cl_abap_behv_test_environment=>create_environment_config(  )->set_bdef_dependencies( bdef_dependencies = VALUE #( ( 'ZAI_DMOI_TRAVEL_U' ) ) ).
*    DATA(bo_double_environment) = cl_abap_behv_test_environment=>create( environment_config = env_config ).
*
*    DATA(bo_double) = bo_double_environment->get_test_double( 'ZAI_DMOI_TRAVEL_U' ).
*
...
*      data(bo_double_input) = bo_double->create_read_input_for_entity( 'ZAI_DMOI_TRAVEL_U'   "can accept the entity name
...
*    ENDLOOP.
  ...
  METHOD set_status_booked_success.
    DATA:
      result      TYPE TABLE     FOR ACTION RESULT ZAI_DMOi_travel_u\\travel~set_status_booked,
      result_line TYPE STRUCTURE FOR ACTION RESULT ZAI_DMOi_travel_u\\travel~set_status_booked.
  ...
  METHOD get_global_authorizations.
    DATA:
      requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST ZAI_DMOi_travel_u\\travel,
      result                   TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT ZAI_DMOi_travel_u\\travel.
...
CLASS ltcl_saver DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    CONSTANTS:
      fm_initialize     TYPE sxco_fm_name VALUE 'ZAI_DMOFLIGHT_TRAVEL_INITIALIZE',
      fm_adjust_numbers TYPE sxco_fm_name VALUE 'ZAI_DMOFLIGHT_TRAVEL_ADJ_NUMBERS',
      fm_save           TYPE sxco_fm_name VALUE 'ZAI_DMOFLIGHT_TRAVEL_SAVE'.
    ...
    DATA:
      ...
      mapped       TYPE RESPONSE FOR MAPPED   LATE ZAI_DMOi_travel_u,
      failed       TYPE RESPONSE FOR FAILED   LATE ZAI_DMOi_travel_u,
      reported     TYPE RESPONSE FOR REPORTED LATE ZAI_DMOi_travel_u.
    ...
    METHODS:
      ...
      "! Checks if { @link ..lsc_i_travel_u.METH:adjust_numbers } mapping from { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_ADJ_NUMBERS } works as expected.
      adjust_numbers    FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lsc_i_travel_u.METH:save } calls { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_SAVE } and does not raise any exception.
      save              FOR TESTING RAISING cx_static_check,

      "! Checks if { @link ..lsc_i_travel_u.METH:cleanup } calls { @link FUNC:ZAI_DMOFLIGHT_TRAVEL_INITIALIZE } and does not raise any exception.
...
*        mapped   = mapped
  ...
  METHOD adjust_numbers.
    CONSTANTS:
      travel_pre_1               TYPE ZAI_DMOtravel_id VALUE '9990',
      travel_pre_2               TYPE ZAI_DMOtravel_id VALUE '9995',
      travel_final_1             TYPE ZAI_DMOtravel_id VALUE '6310',
      travel_final_2             TYPE ZAI_DMOtravel_id VALUE '6315',
      booking_pre_1              TYPE ZAI_DMObooking_id VALUE '990',
      booking_pre_2              TYPE ZAI_DMObooking_id VALUE '995',
      booking_final_1            TYPE ZAI_DMObooking_id VALUE '410',
      booking_final_2            TYPE ZAI_DMObooking_id VALUE '415',
      booking_supplement_pre_1   TYPE ZAI_DMObooking_supplement_id VALUE '90',
      booking_supplement_pre_2   TYPE ZAI_DMObooking_supplement_id VALUE '95',
      booking_supplement_final_1 TYPE ZAI_DMObooking_supplement_id VALUE '10',
      booking_supplement_final_2 TYPE ZAI_DMObooking_supplement_id VALUE '15'.


    DATA:
      travel_mapping       TYPE ZAI_DMOif_flight_legacy=>tt_ln_travel_mapping,
    ...
    travel_mapping = VALUE ZAI_DMOif_flight_legacy=>tt_ln_travel_mapping(
        ( preliminary = VALUE ZAI_DMOif_flight_legacy=>ts_ln_travel( travel_id = travel_pre_1   )
          final       = VALUE ZAI_DMOif_flight_legacy=>ts_ln_travel( travel_id = travel_final_1 ) )
        ( preliminary = VALUE ZAI_DMOif_flight_legacy=>ts_ln_travel( travel_id = travel_pre_2   )
          final       = VALUE ZAI_DMOif_flight_legacy=>ts_ln_travel( travel_id = travel_final_2 ) )
      ).

    booking_mapping = VALUE ZAI_DMOif_flight_legacy=>tt_ln_booking_mapping(
        ( preliminary = VALUE ZAI_DMOif_flight_legacy=>ts_ln_booking( travel_id = travel_pre_1    booking_id = booking_pre_1   )
          final       = VALUE ZAI_DMOif_flight_legacy=>ts_ln_booking( travel_id = travel_final_1  booking_id = booking_final_1 ) )
        ( preliminary = VALUE ZAI_DMOif_flight_legacy=>ts_ln_booking( travel_id = travel_pre_1    booking_id = booking_pre_2   )
          final       = VALUE ZAI_DMOif_flight_legacy=>ts_ln_booking( travel_id = travel_final_1  booking_id = booking_final_2 ) )
      ).

    bookingsuppl_mapping = VALUE ZAI_DMOif_flight_legacy=>tt_ln_bookingsuppl_mapping(
        ( preliminary = VALUE ZAI_DMOif_flight_legacy=>ts_ln_bookingsuppl( travel_id = travel_pre_1    booking_id = booking_pre_1    booking_supplement_id = booking_supplement_pre_1   )
          final       = VALUE ZAI_DMOif_flight_legacy=>ts_ln_bookingsuppl( travel_id = travel_final_1  booking_id = booking_final_1  booking_supplement_id = booking_supplement_final_1 ) )
        ( preliminary = VALUE ZAI_DMOif_flight_legacy=>ts_ln_bookingsuppl( travel_id = travel_pre_1    booking_id = booking_pre_1    booking_supplement_id = booking_supplement_pre_2   )
          final       = VALUE ZAI_DMOif_flight_legacy=>ts_ln_bookingsuppl( travel_id = travel_final_1  booking_id = booking_final_1  booking_supplement_id = booking_supplement_final_2 ) )
      ).
