CLASS ZDMOcl_data_generator_managed DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZDMOCL_DATA_GENERATOR_MANAGED IMPLEMENTATION.
  ...
  METHOD ZDMOif_data_generation_badi~data_generation.
    ...
    DATA max_travel_id TYPE ZDMOtravel_id .

    " Travels
    DATA lt_travel TYPE SORTED TABLE OF ZDMOtravel WITH UNIQUE KEY travel_id.
    SELECT * FROM ZDMOtravel   "#EC CI_ALL_FIELDS_NEEDED
      INTO TABLE @lt_travel.    "#EC CI_NOWHERE

    DATA lt_travel_m TYPE STANDARD TABLE OF ZDMOtravel_m.
    ...
    out->write( ' --> ZDMOTRAVEL_M' ) ##NO_TEXT.
    DELETE FROM ZDMOtravel_m.                          "#EC CI_NOWHERE
    INSERT ZDMOtravel_m FROM TABLE @lt_travel_m.

    out->write( ' --> Set up Number Range Interval' ) ##NO_TEXT.
    CONSTANTS:
      cv_numberrange_interval TYPE cl_numberrange_runtime=>nr_interval VALUE '01',
      cv_numberrange_object   TYPE cl_numberrange_runtime=>nr_object   VALUE 'ZDMOTRV_M' ##NO_TEXT,
    ...
    " bookings
    SELECT * FROM ZDMObooking      "#EC CI_ALL_FIELDS_NEEDED
      INTO TABLE @DATA(lt_booking). "#EC CI_NOWHERE
    DATA lt_booking_m TYPE STANDARD TABLE OF ZDMObooking_m.
    ...
    out->write( ' --> ZDMOBOOKING_M' ) ##NO_TEXT.
    DELETE FROM ZDMObooking_m.                         "#EC CI_NOWHERE
    INSERT ZDMObooking_m FROM TABLE @lt_booking_m.



    " Booking supplements
    DATA lt_booksuppl_m TYPE STANDARD TABLE OF ZDMObooksuppl_m.
    SELECT * FROM ZDMObook_suppl             "#EC CI_ALL_FIELDS_NEEDED
    ...
    out->write( ' --> ZDMOBOOKSUPPL_M' ) ##NO_TEXT.
    DELETE FROM ZDMObooksuppl_m.                       "#EC CI_NOWHERE
