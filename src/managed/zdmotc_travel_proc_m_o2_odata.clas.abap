"!@testing SRVB:ZDMOUI_TRAVEL_PROC_M_O2
...
CLASS ZDMOtc_travel_proc_m_o2_odata DEFINITION
  FOR TESTING
  RISK LEVEL HARMLESS
  DURATION SHORT
  PUBLIC
  ...
  PRIVATE SECTION.
    CLASS-DATA: mo_client_proxy      TYPE REF TO /iwbep/if_cp_client_proxy,
                ...
                begin_date           TYPE ZDMObegin_date,
                end_date             TYPE ZDMOend_date,
                agency_mock_data     TYPE STANDARD TABLE OF ZDMOagency,
                customer_mock_data   TYPE STANDARD TABLE OF ZDMOcustomer,
                carrier_mock_data    TYPE STANDARD TABLE OF ZDMOcarrier,
...
CLASS ZDMOTC_TRAVEL_PROC_M_O2_ODATA IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    mo_client_proxy = create_local_client_proxy( VALUE #( service_id      = 'ZDMOUI_TRAVEL_PROC_M_O2'
                                                          service_version = '0001' )  ).

    " Create the stubs/doubles for the underlying CDS entities
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                      i_for_entities = VALUE #(
                        ( i_for_entity = 'ZDMOC_TRAVEL_PROCESSOR_M'    i_select_base_dependencies = abap_true )
                        ( i_for_entity = 'ZDMOC_BOOKING_PROCESSOR_M'   i_select_base_dependencies = abap_true )
                        ( i_for_entity = 'ZDMOC_BOOKSUPPL_PROCESSOR_M' i_select_base_dependencies = abap_true ) ) ).
...
*        ( 'ZDMOAGENCY' )      "already mocked by cds_test_environment above
*        ( 'ZDMOCUSTOMER' )    "already mocked by cds_test_environment above
*        ( 'ZDMOCARRIER' )     "already mocked by cds_test_environment above
        ( 'ZDMOFLIGHT' )
        ( 'ZDMOSUPPLEMENT' )
  ...
  METHOD create_travel.
    ...
    DATA(ls_business_data) = VALUE ZDMOc_travel_processor_m(
    ...
    DATA ls_response_data TYPE ZDMOc_travel_processor_m.
    ...
    READ ENTITIES OF ZDMOc_travel_processor_m
    ...
    SELECT single @abap_true FROM ZDMOi_travel_m INTO @DATA(lv_travel). "#EC CI_NOWHERE
    cl_abap_unit_assert=>assert_true( msg = 'travel from db' act = lv_travel ).

    " check also log written by "additional save" functionality
    SELECT single @abap_true FROM ZDMOlog_travel INTO @DATA(log_travel). "#EC CI_NOWHERE
    cl_abap_unit_assert=>assert_true( msg = 'ZDMOLOG_TRAVEL' act = log_travel ).
