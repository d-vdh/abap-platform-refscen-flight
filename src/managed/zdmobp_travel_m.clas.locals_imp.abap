...
CLASS lhc_travel DEFINITION INHERITING FROM cl_abap_behavior_handler
  ...
  PRIVATE SECTION.
    ...
    TYPES tt_travel_update TYPE TABLE FOR UPDATE ZDMOI_Travel_M.
...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD validate_customer.
    ...
    READ ENTITIES OF ZDMOI_Travel_M IN LOCAL MODE
    ENTITY travel
     FIELDS ( customer_id )
     WITH CORRESPONDING #(  keys )
    RESULT DATA(travels).

    DATA customers TYPE SORTED TABLE OF ZDMOcustomer WITH UNIQUE KEY customer_id.
    ...
    LOOP AT travels INTO DATA(travel).
      IF travel-customer_id IS INITIAL
        ...
        APPEND VALUE #(  %tky = travel-%tky
                         %msg      = NEW ZDMOcm_flight_messages(
                                         customer_id = travel-customer_id
                                         textid      = ZDMOcm_flight_messages=>customer_unkown
  ...
  METHOD validate_agency.
    ...
    DATA agencies TYPE SORTED TABLE OF ZDMOagency WITH UNIQUE KEY agency_id.
    ...
    IF  agencies IS NOT INITIAL.
      ...
      SELECT FROM ZDMOagency FIELDS agency_id
    ...
    LOOP AT travels INTO DATA(travel).
      IF travel-agency_id IS INITIAL
        ...
        APPEND VALUE #(  %tky = travel-%tky
                         %msg      = NEW ZDMOcm_flight_messages(
                                          textid    = ZDMOcm_flight_messages=>agency_unkown
  ...
  METHOD validate_dates.
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-end_date < travel-begin_date.  "end_date before begin_date
        ...
        APPEND VALUE #( %tky = travel-%tky
                        %msg = NEW ZDMOcm_flight_messages(
                                   textid     = ZDMOcm_flight_messages=>begin_date_bef_end_date
                                   severity   = if_abap_behv_message=>severity-error
      ...
      ELSEIF travel-begin_date < cl_abap_context_info=>get_system_date( ).  "begin_date must be in the future
        ...
        APPEND VALUE #( %tky = travel-%tky
                        %msg = NEW ZDMOcm_flight_messages(
                                    textid   = ZDMOcm_flight_messages=>begin_date_on_or_bef_sysdate
                                    severity = if_abap_behv_message=>severity-error )
                        %element-begin_date  = if_abap_behv=>mk-on
                        %element-end_date    = if_abap_behv=>mk-on
                      ) TO reported-travel.
    ...
    LOOP AT travels INTO DATA(travel).
      CASE travel-overall_status.
        ...
        WHEN OTHERS.
          ...
          APPEND VALUE #( %tky = travel-%tky
                          %msg = NEW ZDMOcm_flight_messages(
                                     textid = ZDMOcm_flight_messages=>status_invalid
                                     severity = if_abap_behv_message=>severity-error
  ...
  METHOD copyTravel.
    ...
    DATA:
      travels       TYPE TABLE FOR CREATE ZDMOI_Travel_M\\travel,
      bookings_cba  TYPE TABLE FOR CREATE ZDMOI_Travel_M\\travel\_booking,
      booksuppl_cba TYPE TABLE FOR CREATE ZDMOI_Travel_M\\booking\_booksupplement.
    ...
    MODIFY ENTITIES OF ZDMOI_Travel_M IN LOCAL MODE
      ENTITY travel
  ...
  METHOD earlynumbering_create.
    ...
    DATA:
      entity        TYPE STRUCTURE FOR CREATE ZDMOI_Travel_M,
      travel_id_max TYPE ZDMOtravel_id.
      ...
      WHEN '1'.
        ...
        LOOP AT entities_wo_travelid INTO entity.
          APPEND VALUE #( %cid = entity-%cid
                          %key = entity-%key
                          %msg = NEW ZDMOcm_flight_messages(
                                      textid = ZDMOcm_flight_messages=>number_range_depleted
      ...
      WHEN '2' OR '3'.
        ...
        LOOP AT entities_wo_travelid INTO entity.
          APPEND VALUE #( %cid = entity-%cid
                          %key = entity-%key
                          %msg = NEW ZDMOcm_flight_messages(
                                      textid = ZDMOcm_flight_messages=>not_sufficient_numbers
                                      severity = if_abap_behv_message=>severity-warning )
                        ) TO reported-travel.
          APPEND VALUE #( %cid        = entity-%cid
                          %key        = entity-%key
  ...
  METHOD earlynumbering_cba_booking.
    DATA: max_booking_id TYPE ZDMObooking_id.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<travel_group>) GROUP BY <travel_group>-travel_id.
      ...
      max_booking_id = REDUCE #( INIT max = CONV ZDMObooking_id( '0' )
                                 FOR  booking IN bookings USING KEY entity WHERE ( source-travel_id  = <travel_group>-travel_id )
                                 NEXT max = COND ZDMObooking_id( WHEN booking-target-booking_id > max
      ...
      max_booking_id = REDUCE #( INIT max = max_booking_id
                                 ...
                                 NEXT max = COND ZDMObooking_id( WHEN   target-booking_id > max
  ...
  METHOD validate_currencycode.
    ...
    LOOP AT travels INTO DATA(travel).
      IF travel-currency_code IS INITIAL.
        ...
        APPEND VALUE #( %tky                   = travel-%tky
                        %msg                   = NEW ZDMOcm_flight_messages(
                                                        textid    = ZDMOcm_flight_messages=>currency_required
                                                        severity  = if_abap_behv_message=>severity-error )
      ...
      ELSEIF NOT line_exists( currency_db[ currency = travel-currency_code ] ).
        ...
        APPEND VALUE #( %tky                   = travel-%tky
                        %msg                   = NEW ZDMOcm_flight_messages(
                                                        textid        = ZDMOcm_flight_messages=>currency_not_existing
                                                        severity      = if_abap_behv_message=>severity-error
                                                        currency_code = travel-currency_code )
                        %element-currency_code = if_abap_behv=>mk-on
                      ) TO reported-travel.
...
********************************************************************************
    ...
    DATA travel_log        TYPE STANDARD TABLE OF ZDMOlog_travel.
    DATA travel_log_create TYPE STANDARD TABLE OF ZDMOlog_travel.
    DATA travel_log_update TYPE STANDARD TABLE OF ZDMOlog_travel.
    ...
    IF create-travel IS NOT INITIAL.
      ...
      " Inserts rows specified in lt_travel_log_c into the DB table ZDMOlog_travel
      INSERT ZDMOlog_travel FROM TABLE @travel_log_create.
    ...
    IF update-travel IS NOT INITIAL.
      ...
      " Inserts rows specified in lt_travel_log_u into the DB table ZDMOlog_travel
      INSERT ZDMOlog_travel FROM TABLE @travel_log_update.
    ...
    IF delete-travel IS NOT INITIAL.
      ...
      " Inserts rows specified in lt_travel_log into the DB table ZDMOlog_travel
      INSERT ZDMOlog_travel FROM TABLE @travel_log.
...
*
********************************************************************************
    DATA booksuppls_db TYPE STANDARD TABLE OF ZDMObooksuppl_m.
    ...
    IF booksuppls_db IS NOT INITIAL.
      ...
      SELECT * FROM ZDMObooksuppl_m FOR ALL ENTRIES IN @booksuppls_db
