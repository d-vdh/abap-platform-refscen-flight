...
CLASS lhc_travel DEFINITION INHERITING FROM cl_abap_behavior_handler
 FRIENDS ltc_travel.
  ...
  PRIVATE SECTION.
    ...
    TYPES:
      t_entities_create TYPE TABLE FOR CREATE ZDMOr_travel_d\\travel,
      t_entities_update TYPE TABLE FOR UPDATE ZDMOr_travel_d\\travel,
      t_failed_travel   TYPE TABLE FOR FAILED   EARLY ZDMOr_travel_d\\travel,
      t_reported_travel TYPE TABLE FOR REPORTED EARLY ZDMOr_travel_d\\travel.
...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD acceptTravel.
    ...
    MODIFY ENTITIES OF ZDMOR_Travel_D IN LOCAL MODE
      ENTITY Travel
    ...
    READ ENTITIES OF ZDMOR_Travel_D IN LOCAL MODE
      ENTITY Travel
        ALL FIELDS WITH
        CORRESPONDING #( keys )
      RESULT DATA(travels).
  ...
  METHOD deductDiscount.
    DATA travels_for_update TYPE TABLE FOR UPDATE ZDMOR_Travel_D.
    ...
    LOOP AT keys_with_valid_discount ASSIGNING FIELD-SYMBOL(<key_with_valid_discount>) WHERE %param-discount_percent IS INITIAL
      ...
      APPEND VALUE #( %tky                       = <key_with_valid_discount>-%tky
                      %msg                       = NEW ZDMOcm_flight_messages(
                                                       textid = ZDMOcm_flight_messages=>discount_invalid
  ...
  METHOD setTravelNumber.
    ...
    "Get max travelID
    SELECT SINGLE FROM ZDMOa_travel_d FIELDS MAX( travel_id ) INTO @DATA(max_travelid).
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS  INITIAL.
        ...
        APPEND VALUE #( %tky                = travel-%tky
                        %state_area         = 'VALIDATE_CUSTOMER'
                        %msg                = NEW ZDMOcm_flight_messages(
                                                                textid   = ZDMOcm_flight_messages=>enter_customer_id
                                                                severity = if_abap_behv_message=>severity-error )
  ...
  METHOD validateAgency.
    ...
    IF  agencies IS NOT INITIAL.
      ...
      SELECT FROM ZDMOagency FIELDS agency_id, country_code
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-AgencyID IS INITIAL.
        ...
        APPEND VALUE #( %tky                = travel-%tky
                        %state_area         = 'VALIDATE_AGENCY'
                        %msg                = NEW ZDMOcm_flight_messages(
                                                          textid   = ZDMOcm_flight_messages=>enter_agency_id
                                                          severity = if_abap_behv_message=>severity-error )
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-BeginDate IS INITIAL.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        %state_area        = 'VALIDATE_DATES'
                         %msg              = NEW ZDMOcm_flight_messages(
                                                                textid   = ZDMOcm_flight_messages=>enter_begin_date
                                                                severity = if_abap_behv_message=>severity-error )
      ...
      IF travel-EndDate IS INITIAL.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        %state_area        = 'VALIDATE_DATES'
                         %msg                = NEW ZDMOcm_flight_messages(
                                                                textid   = ZDMOcm_flight_messages=>enter_end_date
                                                                severity = if_abap_behv_message=>severity-error )
  ...
  METHOD get_global_authorizations.
    ...
    IF requested_authorizations-%create EQ if_abap_behv=>mk-on.
      ...
      ELSE.
        ...
        APPEND VALUE #( %msg    = NEW ZDMOcm_flight_messages(
                                       textid   = ZDMOcm_flight_messages=>not_authorized
                                       severity = if_abap_behv_message=>severity-error )
...
.
  ...
  METHOD is_create_granted.
    ...
    IF country_code IS SUPPLIED.
      AUTHORITY-CHECK OBJECT 'ZDMOTRVL'
        ID 'ZDMOCNTRY' FIELD country_code
        ID 'ACTVT'      FIELD '01'.
...
*      ENDCASE.
  ...
  METHOD get_instance_authorizations.
    ...
    SELECT FROM ZDMOa_travel_d AS travel
      INNER JOIN ZDMOagency    AS agency ON travel~agency_id = agency~agency_id
      ...
      IF sy-subrc = 0.
        ...
        IF update_requested = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky = travel-%tky
                            %msg = NEW ZDMOcm_flight_messages(
                                                     textid    = ZDMOcm_flight_messages=>not_authorized_for_agencyid
                                                     agency_id = travel-AgencyID
                                                     severity  = if_abap_behv_message=>severity-error )
                            %element-AgencyID = if_abap_behv=>mk-on
                           ) TO reported-travel.
  ...
  METHOD precheck_auth.
    DATA:
      ...
      agencies          TYPE SORTED TABLE OF ZDMOagency WITH UNIQUE KEY agency_id,
