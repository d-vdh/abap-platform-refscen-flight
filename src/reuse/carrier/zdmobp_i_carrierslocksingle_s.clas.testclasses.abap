"! @testing BDEF:ZDMOI_CarriersLockSingleton_S
CLASS lthc_carrier DEFINITION FINAL FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.
  PRIVATE SECTION.
    CONSTANTS airlineid_for_test TYPE ZDMOcarrier_id VALUE 'XX4' ##NO_TEXT.
    ...
    DATA:
      failed        TYPE RESPONSE FOR FAILED   EARLY ZDMOi_carrierslocksingleton_s,
      reported      TYPE RESPONSE FOR REPORTED EARLY ZDMOi_carrierslocksingleton_s,
      failed_late   TYPE RESPONSE FOR FAILED   LATE  ZDMOi_carrierslocksingleton_s,
      reported_late TYPE RESPONSE FOR REPORTED LATE  ZDMOi_carrierslocksingleton_s.
...
CLASS lthc_carrier IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
        i_for_entities =  VALUE #(
            ( i_for_entity = 'ZDMOI_CARRIER_S'  )
            ( i_for_entity = 'ZDMOI_Connection' )
          )
      ).
  ENDMETHOD.
  ...
  METHOD validatecurrencycode_success.
    DATA:
      carrier  TYPE ZDMOi_carrier,
      carriers TYPE STANDARD TABLE OF ZDMOi_carrier.

    carrier = VALUE ZDMOi_carrier(
  ...
  METHOD get_instance_features.
    DATA:
      result                     TYPE  TABLE FOR INSTANCE FEATURES RESULT ZDMOi_carrierslocksingleton_s\\carrier,
      carrier_with_connection    TYPE ZDMOi_carrier,
      carrier_without_connection TYPE ZDMOi_carrier,
      carriers                   TYPE STANDARD TABLE OF ZDMOi_carrier,
      connection                 TYPE ZDMOconnection,
      connections                TYPE STANDARD TABLE OF ZDMOconnection.

    carrier_with_connection = VALUE ZDMOi_carrier(
        airlineid = 'AAA'
      ).

    carrier_without_connection = VALUE ZDMOi_carrier(
        airlineid = 'BBB'
      ).
    ...
    connection = VALUE ZDMOconnection(
...
CLASS lthc_carrierslocksingleton IMPLEMENTATION.
  ...
  METHOD get_global_authorizations.
    DATA:
      ...
      requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST ZDMOi_carrierslocksingleton_s\\carrierslocksingleton,
      result                   TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT ZDMOi_carrierslocksingleton_s\\carrierslocksingleton,
      reported                 TYPE RESPONSE  FOR REPORTED EARLY ZDMOi_carrierslocksingleton_s.
...
CLASS ltsc_i_carrierslocksingleton_s DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    DATA:
      ...
      mapped       TYPE RESPONSE FOR MAPPED   LATE ZDMOi_carrierslocksingleton_s,
      failed       TYPE RESPONSE FOR FAILED   LATE ZDMOi_carrierslocksingleton_s,
      reported     TYPE RESPONSE FOR REPORTED LATE ZDMOi_carrierslocksingleton_s.
    ...
    METHODS:
      "! Checks if { @link ..lsc_I_CARRIERSLOCKSINGLETON_S.METH:save_modified } calls { @link FUNC:ZDMOFLIGHT_TRAVEL_SAVE }
