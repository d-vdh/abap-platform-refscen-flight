...
CLASS lhc_agency DEFINITION
  ...
  PRIVATE SECTION.
    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION IMPORTING REQUEST requested_authorizations FOR ZDMOagency RESULT result.

    METHODS validatecountrycode FOR VALIDATE ON SAVE IMPORTING keys FOR ZDMOagency~ZDMOvalidatecountrycode.

    METHODS validateemailaddress FOR VALIDATE ON SAVE IMPORTING keys FOR ZDMOagency~ZDMOvalidateemailaddress.

    METHODS validatename FOR VALIDATE ON SAVE IMPORTING keys FOR ZDMOagency~ZDMOvalidatename.

    METHODS validatelargeobject FOR VALIDATE ON SAVE IMPORTING keys FOR ZDMOagency~ZDMOvalidatelargeobject.
...
CLASS lhc_agency IMPLEMENTATION.
  ...
  METHOD validatecountrycode.
    ...
    READ ENTITIES OF ZDMOr_agencytp IN LOCAL MODE
         ENTITY ZDMOagency
         FIELDS ( countrycode )
    ...
    LOOP AT agencies INTO DATA(agency).
      APPEND VALUE #( %tky        = agency-%tky
             ...
             TO reported-ZDMOagency.

      IF        agency-countrycode IS INITIAL
         OR NOT line_exists( countries_db[ country = agency-countrycode ] ).

        APPEND VALUE #(  %tky = agency-%tky ) TO failed-ZDMOagency.
        APPEND VALUE #( %tky                 = agency-%tky
                        %state_area          = state_area_validate_country
                        %msg                 = NEW ZDMOcx_agency( textid      = ZDMOcx_agency=>country_code_invalid
  ...
  METHOD validateemailaddress.
    ...
    LOOP AT agencies INTO DATA(agency).
      ...
      IF CONV string( agency-emailaddress ) NP '+*@+*.+*'.
        ...
        APPEND VALUE #( %tky                  = agency-%tky
                        %state_area           = state_area_validate_email
                        %msg                  = NEW ZDMOcx_agency( ZDMOcx_agency=>email_invalid_format )
  ...
  METHOD validatename.
    ...
    LOOP AT agencies INTO DATA(agency).
      ...
      IF agency-name IS INITIAL.
        ...
        APPEND VALUE #( %tky          = agency-%tky
                        %state_area   = state_area_validate_name
                        %msg          = NEW ZDMOcx_agency( ZDMOcx_agency=>name_required )
  ...
  METHOD validatelargeobject.
    ...
    LOOP AT agencies INTO DATA(agency).
      ...
      APPEND VALUE #( %tky        = agency-%tky
                      %state_area = state_area_validate_lob ) TO reported-ZDMOagency.
      ...
      IF agency-attachment IS NOT INITIAL AND agency-mimetype IS INITIAL.
        ...
        APPEND VALUE #( %tky              = agency-%tky
                        ...
                        %msg              = NEW ZDMOcx_agency( textid     = ZDMOcx_agency=>mimetype_missing
      ...
      IF agency-mimetype IS NOT INITIAL.
        ...
        IF agency-attachment IS INITIAL.
          ...
          APPEND VALUE #(
              %tky                = agency-%tky
              %state_area         = state_area_validate_lob
              %msg                = NEW ZDMOcx_agency( textid     = ZDMOcx_agency=>attachment_empty_missing
                                                        attachment = agency-attachment )
        ...
        IF NOT line_exists( allowed_mimetypes[ mimetype = agency-mimetype ] ).
          ...
          APPEND VALUE #( %tky              = agency-%tky
                          %state_area       = state_area_validate_lob
                          %msg              = NEW ZDMOcx_agency( textid   = ZDMOcx_agency=>mimetype_not_supported
          ...
          IF agency-filename IS NOT INITIAL.
            ...
            IF file_extension <> allowed_mimetypes[ mimetype = agency-mimetype ]-file_extension.
              ...
              APPEND VALUE #(
                  %tky              = agency-%tky
                  %state_area       = state_area_validate_lob
                  %msg              = NEW ZDMOcx_agency( textid   = ZDMOcx_agency=>extension_mimetype_mismatch
                                                          mimetype = agency-mimetype )
                  %element-mimetype = if_abap_behv=>mk-on
      ...
      ELSEIF agency-attachment IS INITIAL AND agency-filename IS NOT INITIAL.
        ...
        APPEND VALUE #( %tky                = agency-%tky
                        %state_area         = state_area_validate_lob
                        %msg                = NEW ZDMOcx_agency( textid   = ZDMOcx_agency=>only_filename
...
CLASS lsc_agency IMPLEMENTATION.
  ...
  METHOD adjust_numbers.
    DATA agency_id_max TYPE ZDMOagency_id.
    DATA entity        TYPE STRUCTURE FOR MAPPED LATE ZDMOr_agencytp.

    DATA(entities_wo_agencyid) = mapped-ZDMOagency.
    ...
    CASE number_range_return_code.
      WHEN '1'.
        ...
        LOOP AT entities_wo_agencyid INTO entity.
          APPEND VALUE #( %pid = entity-%pid
                          %key = entity-%key
                          %msg = NEW ZDMOcx_agency( textid   = ZDMOcx_agency=>number_range_depleted
      ...
      WHEN '2' OR '3'.
        ...
        RAISE SHORTDUMP NEW ZDMOcx_agency( textid   = ZDMOcx_agency=>not_sufficient_numbers
                                            severity = if_abap_behv_message=>severity-warning ).
    ENDCASE.
    ...
    " Set Agency ID
    LOOP AT mapped-ZDMOagency ASSIGNING FIELD-SYMBOL(<agency>). " USING KEY entity" WHERE agencyid IS INITIAL.
