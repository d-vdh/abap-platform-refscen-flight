...
CLASS ltc_agency_handler DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA mapped        TYPE RESPONSE FOR MAPPED EARLY ZDMOr_agencytp.
    DATA failed        TYPE RESPONSE FOR FAILED EARLY ZDMOr_agencytp.
    DATA reported      TYPE RESPONSE FOR REPORTED EARLY ZDMOr_agencytp.
    DATA failed_late   TYPE RESPONSE FOR FAILED LATE ZDMOr_agencytp.
    DATA reported_late TYPE RESPONSE FOR REPORTED LATE ZDMOr_agencytp.
...
CLASS ltc_agency_handler IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    sql_test_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( 'ZDMOR_AgencyTP' ) ( 'I_COUNTRY' ) ) ).
  ENDMETHOD.
  ...
  METHOD validate_name_success.
    DATA agency   TYPE ZDMOr_agencytp.
    DATA agencies TYPE STANDARD TABLE OF ZDMOr_agencytp.

    agency = VALUE ZDMOr_agencytp( agencyid = '123'
                                    name     = 'FlightBooker' ).
    ...
    cl_abap_unit_assert=>assert_equals( msg = 'Reported has more than one messages'
                                        exp = 1
                                        act = lines( reported_late-ZDMOagency ) ).
    DATA(reported_agency) = reported_late-ZDMOagency[ 1 ].
  ...
  METHOD validate_name_failed.
    DATA agency                    TYPE ZDMOr_agencytp.
    DATA agencies                  TYPE STANDARD TABLE OF ZDMOr_agencytp.
    DATA reported_with_message     TYPE STRUCTURE FOR REPORTED LATE ZDMOr_agencytp.
    DATA reported_clear_state_area TYPE STRUCTURE FOR REPORTED LATE ZDMOr_agencytp.
    ...
    cl_abap_unit_assert=>assert_equals( exp = 1  act = lines( failed_late-ZDMOagency ) ).

    LOOP AT failed-ZDMOagency INTO DATA(failed_line).
    ...
    LOOP AT reported_late-ZDMOagency INTO DATA(reported_line).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = CORRESPONDING symsg( ZDMOcx_agency=>name_required )
  ...
  METHOD validate_email_failed.
    CONSTANTS cv_found TYPE ZDMOr_agencytp-name VALUE 'found' ##NO_TEXT.
    DATA agency                    TYPE ZDMOr_agencytp.
    DATA agencies                  TYPE STANDARD TABLE OF ZDMOr_agencytp.
    DATA reported_with_message     TYPE STRUCTURE FOR REPORTED LATE ZDMOr_agencytp.
    DATA reported_clear_state_area TYPE STRUCTURE FOR REPORTED LATE ZDMOr_agencytp.
    ...
    cl_abap_unit_assert=>assert_equals( exp = lines( agencies )
                                        act = lines( failed_late-ZDMOagency ) ).
    ...
    LOOP AT agencies ASSIGNING FIELD-SYMBOL(<agency>).
      ...
      LOOP AT reported_late-ZDMOagency INTO DATA(reported_line) USING KEY entity WHERE agencyid = <agency>-agencyid.
        IF reported_line-%msg IS BOUND.
          reported_with_message     = reported_line.
        ELSE.
          reported_clear_state_area = reported_line.
      ...
      cl_abap_unit_assert=>assert_equals(
          exp = CORRESPONDING symsg( ZDMOcx_agency=>email_invalid_format )
          act = CORRESPONDING symsg( reported_with_message-%msg->if_t100_message~t100key ) ).
    ENDLOOP.
...
*    cds_test_environment->insert_test_data( agencies ).
  ...
  METHOD validate_countrycode_failed.
    ...
    LOOP AT agencies ASSIGNING FIELD-SYMBOL(<agency>).
      ...
      cl_abap_unit_assert=>assert_equals(
          exp = CORRESPONDING symsg( ZDMOcx_agency=>country_code_invalid )
          act = CORRESPONDING symsg( reported_with_message-%msg->if_t100_message~t100key ) ).
    ENDLOOP.
  ...
  METHOD validate_lob_success.
    ...
    cl_abap_unit_assert=>assert_equals( exp = 3
                                        act = lines( reported_late-ZDMOagency ) ).
    DATA(reported_agency) = reported_late-ZDMOagency.
  ...
  METHOD validate_lob_failed.
    ...
    LOOP AT failed_late-ZDMOagency INTO DATA(failed_line).
      cl_abap_unit_assert=>assert_not_initial( msg = 'Failed key was not provided'
                                               act = VALUE #( agencies[ agencyid = failed_line-agencyid ] OPTIONAL ) ).
    ENDLOOP.
    ...
    " check messages
    cl_abap_unit_assert=>assert_equals( exp = if_abap_behv=>mk-on
                                        act = reported_late-ZDMOagency[ 2 ]-%element-mimetype ).
    cl_abap_unit_assert=>assert_equals(
        exp = CORRESPONDING symsg( ZDMOcx_agency=>mimetype_missing )
        act = CORRESPONDING symsg( reported_late-ZDMOagency[ 2 ]-%msg->if_t100_message~t100key ) ).


    cl_abap_unit_assert=>assert_equals( exp = if_abap_behv=>mk-on
                                        act = reported_late-ZDMOagency[ 4 ]-%element-attachment ).
    cl_abap_unit_assert=>assert_equals(
        exp = CORRESPONDING symsg( ZDMOcx_agency=>attachment_empty_missing )
        act = CORRESPONDING symsg( reported_late-ZDMOagency[ 4 ]-%msg->if_t100_message~t100key ) ).


    cl_abap_unit_assert=>assert_equals( exp = if_abap_behv=>mk-on
                                        act = reported_late-ZDMOagency[ 6 ]-%element-attachment ).
    cl_abap_unit_assert=>assert_equals( exp = if_abap_behv=>mk-on
                                        act = reported_late-ZDMOagency[ 6 ]-%element-mimetype ).
    cl_abap_unit_assert=>assert_equals(
        exp = CORRESPONDING symsg( ZDMOcx_agency=>only_filename )
        act = CORRESPONDING symsg( reported_late-ZDMOagency[ 6 ]-%msg->if_t100_message~t100key ) ).


    cl_abap_unit_assert=>assert_equals( exp = if_abap_behv=>mk-on
                                        act = reported_late-ZDMOagency[ 8 ]-%element-mimetype ).
    cl_abap_unit_assert=>assert_equals(
        exp = CORRESPONDING symsg( ZDMOcx_agency=>mimetype_missing )
        act = CORRESPONDING symsg( reported_late-ZDMOagency[ 8 ]-%msg->if_t100_message~t100key ) ).


    cl_abap_unit_assert=>assert_equals( exp = if_abap_behv=>mk-on
                                        act = reported_late-ZDMOagency[ 10 ]-%element-attachment ).
    cl_abap_unit_assert=>assert_equals(
        exp = CORRESPONDING symsg( ZDMOcx_agency=>attachment_empty_missing )
        act = CORRESPONDING symsg( reported_late-ZDMOagency[ 10 ]-%msg->if_t100_message~t100key ) ).


    cl_abap_unit_assert=>assert_equals( exp = if_abap_behv=>mk-on
                                        act = reported_late-ZDMOagency[ 12 ]-%element-mimetype ).
    cl_abap_unit_assert=>assert_equals(
        exp = CORRESPONDING symsg( ZDMOcx_agency=>mimetype_not_supported )
        act = CORRESPONDING symsg( reported_late-ZDMOagency[ 12 ]-%msg->if_t100_message~t100key ) ).


    cl_abap_unit_assert=>assert_equals( exp = if_abap_behv=>mk-on
                                        act = reported_late-ZDMOagency[ 14 ]-%element-filename ).
    cl_abap_unit_assert=>assert_equals(
        exp = CORRESPONDING symsg( ZDMOcx_agency=>extension_mimetype_mismatch )
        act = CORRESPONDING symsg( reported_late-ZDMOagency[ 14 ]-%msg->if_t100_message~t100key ) ).


    cl_abap_unit_assert=>assert_equals( exp = if_abap_behv=>mk-on
                                        act = reported_late-ZDMOagency[ 16 ]-%element-mimetype ).
    cl_abap_unit_assert=>assert_equals(
        exp = CORRESPONDING symsg( ZDMOcx_agency=>mimetype_not_supported )
        act = CORRESPONDING symsg( reported_late-ZDMOagency[ 16 ]-%msg->if_t100_message~t100key ) ).

  ENDMETHOD.


  METHOD get_global_authorizations.
    DATA requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST ZDMOr_agencytp\\ZDMOagency.
    DATA result                   TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT ZDMOr_agencytp\\ZDMOagency.
    DATA reported                 TYPE RESPONSE FOR REPORTED EARLY ZDMOr_agencytp.
...
CLASS ltc_agency_saver DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA mapped   TYPE RESPONSE FOR MAPPED LATE ZDMOr_agencytp.
    DATA reported TYPE RESPONSE FOR REPORTED LATE ZDMOr_agencytp.
...
CLASS ltc_agency_saver IMPLEMENTATION.
  ...
  METHOD latenumbering_idempotency.
    CONSTANTS cv_agencyid TYPE ZDMOr_agencytp-agencyid VALUE '123'.

    DATA act_mapped_line LIKE LINE OF mapped-ZDMOagency.
    DATA exp_mapped_line LIKE LINE OF mapped-ZDMOagency.

    mapped-ZDMOagency = VALUE #( ( agencyid = cv_agencyid ) ).
    ...
    cl_abap_unit_assert=>assert_equals( exp = 1
                                        act = lines( mapped-ZDMOagency ) ).

    cl_abap_unit_assert=>assert_equals( exp = cv_agencyid
                                        act = mapped-ZDMOagency[ 1 ]-agencyid ).
  ENDMETHOD.
  ...
  METHOD latenumbering_new_number.
    ...
    mapped-ZDMOagency = VALUE #( ( %pid = cv_pid ) ).
    ...
    cl_abap_unit_assert=>assert_equals( exp = cv_pid
                                        act = mapped-ZDMOagency[ 1 ]-%pid ).

    cl_abap_unit_assert=>assert_not_initial( mapped-ZDMOagency[ 1 ]-agencyid ).
  ENDMETHOD.
