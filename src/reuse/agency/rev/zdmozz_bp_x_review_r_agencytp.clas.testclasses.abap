"! @testing BDEF:ZDMOZZ_X_REVIEW_R_AGENCYTP
CLASS ltcl_review DEFINITION FINAL FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.

  PRIVATE SECTION.
    CLASS-DATA class_under_test     TYPE REF TO lhc_ZDMOzz_review.
    ...
    "! Calls { @link ..lhc_ZDMOZZ_Review.METH:get_instance_features }
    "! with all possible in- and outcomes.
    METHODS get_instance_features     FOR TESTING.

    "! Checks that { @link ..lhc_ZDMOZZ_Review.METH:get_global_authorizations } returns initial values
...
CLASS ltcl_review IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                               VALUE #( ( i_for_entity = 'ZDMOI_AGENCYTP'            )
                                        ( i_for_entity = 'ZDMOZZ_I_AGENCY_REVIEWTP'  ) ) ).
  ENDMETHOD.
  ...
  METHOD get_instance_features.
    TYPES: t_instance_feature TYPE STRUCTURE FOR INSTANCE FEATURES RESULT ZDMOr_agencytp\\ZDMOzz_review,
           BEGIN OF t_check.
    INCLUDE TYPE t_instance_feature.
    TYPES: reviewer TYPE ZDMOzz_r_agency_reviewtp-reviewer,
           END OF t_check,
           t_check_table TYPE STANDARD TABLE OF t_check WITH KEY agencyid reviewid.

    DATA check_table        TYPE t_check_table.
    DATA review_mock_data   TYPE STANDARD TABLE OF ZDMOzz_r_agency_reviewtp.
    DATA reviews_to_test    TYPE TABLE FOR INSTANCE FEATURES KEY ZDMOi_agencytp\\ZDMOzz_review.
    DATA requested_features TYPE STRUCTURE FOR INSTANCE FEATURES REQUEST ZDMOi_agencytp\\ZDMOzz_review.
    DATA act_result         TYPE TABLE FOR INSTANCE FEATURES RESULT ZDMOi_agencytp\\ZDMOzz_review.
    DATA exp_result         TYPE TABLE FOR INSTANCE FEATURES RESULT ZDMOi_agencytp\\ZDMOzz_review.
    DATA reported           TYPE RESPONSE FOR REPORTED EARLY ZDMOi_agencytp.
    DATA failed             TYPE RESPONSE FOR FAILED EARLY ZDMOi_agencytp.
  ...
  METHOD get_global_authorizations.
    DATA requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST ZDMOi_agencytp\\ZDMOzz_review.
    DATA result                   TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT ZDMOi_agencytp\\ZDMOzz_review.
    DATA reported                 TYPE RESPONSE FOR REPORTED EARLY ZDMOi_agencytp.

    requested_authorizations-%action-ZDMOreviewwashelpful    = if_abap_behv=>mk-on.
    requested_authorizations-%action-ZDMOreviewwasnothelpful = if_abap_behv=>mk-on.
  ...
  METHOD ratinginrange_valid.
    ...
    DATA reported TYPE RESPONSE FOR REPORTED LATE ZDMOi_agencytp.
    ...
    cl_abap_unit_assert=>assert_equals( msg = 'Reported has not the correct amount of messages'
                                        ...
                                        act = lines( reported-ZDMOzz_review ) ).
  ENDMETHOD.


  METHOD ratinginrange_invalid.
    TYPES BEGIN OF ts_review_test_data.
            INCLUDE TYPE ZDMOzz_r_agency_reviewtp.
    ...
    DATA review_mock_data          TYPE STANDARD TABLE OF ZDMOzz_r_agency_reviewtp.
    DATA reported                  TYPE RESPONSE FOR REPORTED LATE ZDMOi_agencytp.
    DATA reported_with_message     TYPE STRUCTURE FOR REPORTED LATE ZDMOi_agencytp\\ZDMOzz_review.
    DATA reported_clear_state_area TYPE STRUCTURE FOR REPORTED LATE ZDMOi_agencytp\\ZDMOzz_review.

    review_test_data = VALUE #(
        agencyid = '1'
        ( reviewid = '1'  rating = '0'  t100 = CORRESPONDING #( ZDMOzz_cx_agency_review=>rating_invalid ) )
        ( reviewid = '2'  rating = '7'  t100 = CORRESPONDING #( ZDMOzz_cx_agency_review=>rating_invalid ) ) ).
    ...
    LOOP AT review_test_data ASSIGNING FIELD-SYMBOL(<reviewy>).
      ...
      LOOP AT reported-ZDMOzz_review INTO DATA(reported_line) USING KEY entity
        ...
        cl_abap_unit_assert=>assert_equals( exp = lhc_ZDMOzz_review=>rating_in_range
  ...
  METHOD _reviewhelpful.
    DATA review_mock_data TYPE STANDARD TABLE OF ZDMOzz_r_agency_reviewtp.
    DATA act_reviews      TYPE TABLE FOR READ RESULT ZDMOi_agencytp\\ZDMOzz_review.
    DATA exp_reviews      TYPE TABLE FOR READ RESULT ZDMOi_agencytp\\ZDMOzz_review.
    ...
    READ ENTITIES OF ZDMOi_agencytp
...
CLASS ltcl_sc_r_agency DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    CONSTANTS agency_event  TYPE c length 30 VALUE 'ZDMOAGENCYREVIEWCREATED'.
    CONSTANTS agency_entity TYPE abp_entity_name VALUE 'ZDMOI_AGENCYTP'.
...
CLASS ltcl_sc_r_agency IMPLEMENTATION.
  ...
  METHOD class_setup.
    CREATE OBJECT class_under_test FOR TESTING.
    sql_test_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( 'ZDMOZZ_AGN_REVA' ) ) ).
  ...
  METHOD adjust_numbers.
    DATA act_mapped       TYPE RESPONSE FOR MAPPED LATE ZDMOi_agencytp.
    DATA exp_mapped       TYPE RESPONSE FOR MAPPED LATE ZDMOi_agencytp.
    DATA review_mock_data TYPE STANDARD TABLE OF ZDMOzz_agn_reva.
    DATA reported         TYPE RESPONSE FOR REPORTED LATE ZDMOi_agencytp.
    ...
    exp_mapped-ZDMOzz_review = VALUE #(
    ...
    act_mapped-ZDMOzz_review = CORRESPONDING #( exp_mapped-ZDMOzz_review MAPPING %pid = %pid  %tmp = %tmp EXCEPT * ).
  ...
  METHOD save_md_one_review_created.
    ...
    " Prepare
    DATA agency_mock_data TYPE STANDARD TABLE OF ZDMOagency.

    agency_mock_data = VALUE #( ( agency_id = '1' email_address = 'test@test.example' ) ).
    cds_test_environment->insert_test_data( agency_mock_data ).

    DATA create TYPE REQUEST FOR CHANGE ZDMOi_agencytp.
    create-ZDMOzz_review = VALUE #( ( agencyid        = '1'
                                       reviewid        = '1'
                                       rating          = '1'
                                       freetextcomment = 'Bad' ) ).
    DATA reported TYPE RESPONSE FOR REPORTED LATE ZDMOi_agencytp.
    ...
    DATA event_payload_act TYPE TABLE FOR EVENT ZDMOi_agencytp~ZDMOagencyreviewcreated.
    event_payload_act = event_test_environment->get_event( entity_name  = agency_entity event_name = agency_event )->get_payload( )->*.

    DATA event_payload_exp TYPE TABLE FOR EVENT ZDMOi_agencytp~ZDMOagencyreviewcreated.
