"! @testing BDEF:ZDMOZZ_X_COUNTRY_R_AGENCYTP
...
CLASS ltcl_agency_w_cds_tdf IMPLEMENTATION.
  ...
  METHOD validatecountry_invalidnumber.
    ...
    DATA:
      ...
      reported                  TYPE RESPONSE FOR REPORTED LATE  ZDMOi_agencytp,
      reported_with_message     TYPE STRUCTURE FOR REPORTED LATE ZDMOi_agencytp,
      reported_clear_state_area TYPE STRUCTURE FOR REPORTED LATE ZDMOi_agencytp.

    agency_test_data = VALUE #(
        ( agency_id = '1' phone_number = '49'     country_code = 'DE'  t100 = CORRESPONDING #( ZDMOzz_cx_agency_country=>number_invalid      ) )
        ( agency_id = '2' phone_number = '49'     country_code = 'D'   t100 = CORRESPONDING #( ZDMOzz_cx_agency_country=>number_invalid      ) )
        ( agency_id = '3' phone_number = '+49'    country_code = 'D'   t100 = CORRESPONDING #( ZDMOzz_cx_agency_country=>combination_invalid ) )
        ( agency_id = '4' phone_number = '008955' country_code = 'DE'  t100 = CORRESPONDING #( ZDMOzz_cx_agency_country=>combination_invalid ) )
      ).
    ...
    LOOP AT agency_test_data ASSIGNING FIELD-SYMBOL(<agency>).
      ...
      LOOP AT reported-ZDMOagency INTO DATA(reported_line) USING KEY entity WHERE agencyid = <agency>-agency_id.
  ...
  METHOD determinecountrycode.
    TYPES BEGIN OF ts_agency_test_data.
    INCLUDE TYPE ZDMOagency.
    TYPES exp_country_code TYPE ZDMOagency-country_code.
    TYPES END OF ts_agency_test_data.
    TYPES tt_agency_test_data TYPE STANDARD TABLE OF ts_agency_test_data.
  ...
  METHOD determinediallingcode.
    TYPES BEGIN OF ts_agency_test_data.
    INCLUDE TYPE ZDMOagency.
    TYPES exp_phone_number TYPE ZDMOagency-phone_number.
    TYPES END OF ts_agency_test_data.
    TYPES tt_agency_test_data TYPE STANDARD TABLE OF ts_agency_test_data.
  ...
  METHOD get_global_authorizations.
    DATA:
      requested_authorizations TYPE STRUCTURE FOR GLOBAL AUTHORIZATION REQUEST ZDMOi_agencytp\\ZDMOagency,
      result                   TYPE STRUCTURE FOR GLOBAL AUTHORIZATION RESULT ZDMOi_agencytp\\ZDMOagency,
      reported                 TYPE RESPONSE  FOR REPORTED EARLY ZDMOi_agencytp.

    requested_authorizations-%action-ZDMOcreatefromtemplate = if_abap_behv=>mk-on.
  ...
  METHOD createfromtemplate_valid.
    ...
    DATA:
      agency_mock_data TYPE STANDARD TABLE OF ZDMOagency,
      mapped           TYPE RESPONSE FOR MAPPED EARLY ZDMOi_agencytp,
      reported         TYPE RESPONSE FOR REPORTED EARLY ZDMOi_agencytp,
      failed           TYPE RESPONSE FOR FAILED EARLY ZDMOi_agencytp.

    DATA(agency_to_test) = VALUE ZDMOagency(
        agency_id             = '1'
        name                  = 'Test'
        street                = 'Street'
        postal_code           = '123'
    ...
    cl_abap_unit_assert=>assert_initial( mapped-ZDMOzz_review ).

    cl_abap_unit_assert=>assert_not_initial( mapped-ZDMOagency ).
    cl_abap_unit_assert=>assert_equals(
        act = lines( mapped-ZDMOagency )
        exp = 1
      ).

    DATA(mapped_line) = mapped-ZDMOagency[ 1 ].
    cl_abap_unit_assert=>assert_equals(
        act = mapped_line-%cid
        exp = cid
      ).
    ...
    READ ENTITIES OF ZDMOi_agencytp
      ENTITY ZDMOagency
        FIELDS (
          ...
          filename
          ZDMOzzsloganzag
        ) WITH CORRESPONDING #( mapped-ZDMOagency )
  ...
  METHOD createfromtemplate_invalid.
    ...
    cl_abap_unit_assert=>assert_initial( failed-ZDMOzz_review ).

    cl_abap_unit_assert=>assert_equals(
        act = lines( failed-ZDMOagency )
        exp = 1
      ).

    cl_abap_unit_assert=>assert_equals(
        act = failed-ZDMOagency[ 1 ]-%fail-cause
