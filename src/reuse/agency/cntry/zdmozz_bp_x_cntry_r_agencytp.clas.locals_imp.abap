...
CLASS lhc_Agency DEFINITION INHERITING FROM cl_abap_behavior_handler
  ...
  PRIVATE SECTION.
    ...
    METHODS validateDiallingCode FOR VALIDATE ON SAVE
      IMPORTING keys FOR ZDMOAgency~ZDMOvalidateDiallingCode.
    METHODS determineCountryCode FOR DETERMINE ON MODIFY
      IMPORTING keys FOR ZDMOAgency~ZDMOdetermineCountryCode.
    METHODS determineDiallingCode FOR DETERMINE ON MODIFY
      IMPORTING keys FOR ZDMOAgency~ZDMOdetermineDiallingCode.
    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR ZDMOAgency RESULT result.
    METHODS createFromTemplate FOR MODIFY
      IMPORTING keys FOR ACTION ZDMOAgency~ZDMOcreateFromTemplate.
...
CLASS lhc_Agency IMPLEMENTATION.
  ...
  METHOD validateDiallingCode.
    ...
    LOOP AT agencies INTO DATA(agency).
      APPEND VALUE #( %tky        = agency-%tky
                      %state_area = validate_dialling_code ) TO reported-ZDMOAgency.
      ...
      IF agency-PhoneNumber(1) <> '+'.
        ...
        APPEND VALUE #( %tky        = agency-%tky
                        ...
                        %msg        = NEW ZDMOzz_cx_agency_country( textid      = ZDMOzz_cx_agency_country=>number_invalid
      ...
      AND NOT line_exists( countries[ number = agency-phonenumber(4) code = agency-CountryCode ] ).
        APPEND VALUE #( %tky        = agency-%tky
                        %state_area = validate_dialling_code
                        %msg        = NEW ZDMOzz_cx_agency_country( textid      = ZDMOzz_cx_agency_country=>combination_invalid )
                        %element-PhoneNumber = if_abap_behv=>mk-on )
                        TO reported-ZDMOAgency.
  ...
  METHOD determineCountryCode.
    ...
    DATA: agencies_to_update TYPE TABLE FOR UPDATE ZDMOi_agencytp.
  ...
  METHOD createFromTemplate.
    ...
    DATA: agencies_to_create TYPE TABLE FOR CREATE ZDMOI_AgencyTP.
    LOOP AT agencies INTO DATA(agency).
