...
CLASS lhc_supplement IMPLEMENTATION.
  ...
  METHOD validateprice.
    READ ENTITIES OF ZDMOi_supplement IN LOCAL MODE
      ENTITY supplement
    ...
    LOOP AT supplements INTO DATA(supplement).
      ...
      IF supplement-price IS INITIAL.
        ...
        APPEND VALUE #( %tky           = supplement-%tky
                        ...
                        %msg           = NEW ZDMOcx_supplement(
                                             textid    = ZDMOcx_supplement=>price_required
      ...
      IF supplement-currencycode IS INITIAL.
        ...
        APPEND VALUE #( %tky                 = supplement-%tky
                        %state_area          = 'VALIDATE_PRICE'
                        %msg                 = NEW ZDMOcx_supplement(
                                                      textid    = ZDMOcx_supplement=>currency_required
                                                      severity  = if_abap_behv_message=>severity-error )
      ...
      ELSEIF NOT line_exists( currency_db[ currency = supplement-currencycode ] ).
        ...
        APPEND VALUE #( %tky                 = supplement-%tky
                        %state_area          = 'VALIDATE_PRICE'
                        %msg                 = NEW ZDMOcx_supplement(
                                                      textid    = ZDMOcx_supplement=>currency_not_existing
                                                      severity  = if_abap_behv_message=>severity-error )
                        %element-currencycode = if_abap_behv=>mk-on
                      ) TO reported-supplement.
      ENDIF.
  ...
  METHOD earlynumbering_create.
    DATA: entities_wo_supplementid     TYPE TABLE FOR CREATE ZDMOi_supplement,
          entities_wo_supplid_filtered TYPE TABLE FOR CREATE ZDMOi_supplement,
          entity                       TYPE STRUCTURE FOR CREATE ZDMOi_supplement.
    ...
    LOOP AT entities_wo_supplementid INTO DATA(supplement) GROUP BY supplement-supplementcategory INTO DATA(supplementcategory).
      ...
      CASE number_range_return_code.
        WHEN '1'.
          ...
          APPEND NEW ZDMOcx_supplement(
                              textid          = ZDMOcx_supplement=>numbers_left
        ...
        WHEN '2' OR '3'.
          ...
          LOOP AT entities_wo_supplid_filtered INTO entity.
            APPEND VALUE #(
                  ...
                  %key      = entity-%key
                  %msg      = NEW ZDMOcx_supplement(
                                    textid              = ZDMOcx_supplement=>numbers_last
                                    severity            = if_abap_behv_message=>severity-error
                                    supplement_category = supplementcategory )
              ) TO reported-supplement.
            APPEND VALUE #(
