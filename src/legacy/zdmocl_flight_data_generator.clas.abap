CLASS ZDMOcl_flight_data_generator DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    CLASS-METHODS:
      ...
      "!  <br/>
      "! Propagation to ZDMOcl_flight_legacy=>calculate_flight_price.<br/>
      ...
      calculate_flight_price
        ...
        RETURNING
          VALUE(rv_price)           TYPE ZDMOflight_price.
...
CLASS ZDMOCL_FLIGHT_DATA_GENERATOR IMPLEMENTATION.
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    out->write( 'Generate Data: Airport      ZDMOAIRPORT' ) ##NO_TEXT.
    lcl_airport_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Carrier      ZDMOCARRIER' ) ##NO_TEXT.
    lcl_carrier_data_generator=>lif_data_generator~create( out ).
    out->write( 'Generate Data: Connection   ZDMOCONNECTION' ) ##NO_TEXT.
    lcl_connection_data_generator=>lif_data_generator~create( out ).
    out->write( 'Generate Data: Flight       ZDMOFLIGHT' ) ##NO_TEXT.
    lcl_flight_data_generator=>lif_data_generator~create( out ).


    out->write( 'Generate Data: Agency       ZDMOAGENCY' ) ##NO_TEXT.
    lcl_agency_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Customer      ZDMOCUSTOMER' ) ##NO_TEXT.
    lcl_customer_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Supplement      ZDMOSUPPLEMENT' ) ##NO_TEXT.
    lcl_supplement_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Travel      ZDMOTRAVEL' ) ##NO_TEXT.
    out->write( 'Generate Data: Booking      ZDMOBOOKING' ) ##NO_TEXT.
    out->write( 'Generate Data: Booking Supplement      ZDMOBOOK_SUPPL' ) ##NO_TEXT.
    ...
    DATA lo_badi TYPE REF TO ZDMOdata_generation_badi.
    GET BADI lo_badi.
  ...
  METHOD calculate_flight_price.
    rv_price = ZDMOcl_flight_legacy=>calculate_flight_price(
