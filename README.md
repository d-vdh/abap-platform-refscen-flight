...
1. In your ABAP cloud project, create the ABAP package `ZAI_DMOFLIGHT` (using the superpackage `ZAI_DMOSAP`) as the target package for the demo content to be downloaded (leave the suggested values unchanged when following the steps in the package creation wizard).
...
5. Select the branch <em>ABAP-platform-cloud</em> and enter the newly created package `ZAI_DMOFLIGHT` as the target package and choose <em>Next</em>.
...
## Configuration
To activate all development objects from the `ZAI_DMOFLIGHT` package: 
...
3. Service definitions need a provider contract before they can be released for the release contract <em>Extend (C0)</em>. The service definition ZAI_DMOUI_AGENCY from the package ZAI_DMOFLIGHT_REUSE_AGENCY is shipped without this release contract for maintenance reasons and does not contain a provider contract. If you want to release the service definition ZAI_DMOUI_AGENCY for the release contract <em>Extend (C0)</em>, you need to define a suitable provider contract first. You can also directly copy the source code from [service_definition_agency](service_definition_agency). Activate the service definition after.
...
1. Expand the package structure in the Project Explorer `ZAI_DMOFLIGHT_LEGACY` > `Source Code Library` > `Classes`.
2. Select the data generator class `ZAI_DMOCL_FLIGHT_DATA_GENERATOR` and press `F9` (Run as Console Application). 

NOTE: The namespace ZAI_DMO is reserved for the demo content. Apart from the downloaded demo content, do not use the namespace ZAI_DMO and do not create any development objects in the downloaded packages. You can access the development objects in ZAI_DMO from your own namespace.
